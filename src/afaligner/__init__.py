...
def align(
    ...
    It takes a list of text files and a list of audio files with the same text
    narrated and returns a mapping from the text fragments (paragraph,
    sentence, word) to the their corresponding places in the audio (begin_time,
    end_time). For alignment details see build_sync_map function.
    ...
    `times_as_timedelta` – if True, returned times are datetime.timedelta()
    instances. If False, the times are strings of the format
    f'{hours:d}:{minutes:0>2d}:{seconds:0>2d}.{ms:0>3d}'

    `language` - language of the text. One of aenas.language.Language. Defaults
    to `Language.ENG`
    ...
    If `output_dir` is not None, outputs sync map formatted according to
    `output_format`.
    ...
    text_paths = (os.path.join(text_dir, f)
                  for f in sorted(os.listdir(text_dir)))
    audio_paths = (os.path.join(audio_dir, f)
...
def build_sync_map(
    ...
    """
    This is an algorithm for building a sync map. It synthesizes text and
    then aligns synthesized audio with the recorded audio using a variation of
    the DTW (Dynamic Time Warping) algorithm.

    The main features of this algorithm are:

    1) It can handle structural differences in the beginning and in the end of
    files.
    2) It finds an approximation to an optimal warping path in linear
    time and space using FastDTW approach.
    ...
    the quality of the result is sensitive to the choice of skip_penalty and
    radius parameters, so it is recommended to have such a correspondance.

    Alignment details:

    Synthesized and recorded audio are represented as sequences of MFCC frames.
    These sequences are aligned using variation of the DTW algorithm. In
    contrast to the classic DTW, this algorithms can be used to align sequences
    with structural differences in the beginning or in the end.

    Steps to build a sync map:

    1) Synthesize text file and produce a list of anchors. Each anchor
    represents the start of the corresponding text fragment in a synthesized
    audio.
    ...
    5) Map anchors inside the boundaries to the recorded MFCC sequence using
    warping path from step 3.

    6) Start all over again considering:

       - If there is an extra content in the end of synthesized sequence, align
         it with the next audio file.

       - If there is an extra content in the end of recorded sequence, align it
         with the next text file.

       - If both sequences have extra content in the end, align text tail with
         the next audio file.
    ...
    while True:
        if process_next_text:
            ...
            output_text_name = os.path.join(sync_map_text_path_prefix,
                                            text_name)
            textfile = TextFile(text_path, file_format=TextFileFormat.UNPARSED,
                                parameters=parse_parameters)
            textfile.set_language(language)
            text_wav_path = os.path.join(
                tmp_dir,
                f'{drop_extension(text_name)}_text.wav'
            )
            ...
            anchors = np.array([int(a[0] / TimeValue('0.040'))
                                for a in anchors])

            # MFCC frames sequence memory layout is a n x l 2D array, where n -
            # number of frames and l - number of MFFCs # i.e it is
            # c-contiguous, but after dropping the first coefficient it siezes
            # to be c-contiguous.
            #
            # Should decide whether to make a copy or to work around the first
            # coefficient.
        ...
        if process_next_audio:
            ...
            output_audio_name = os.path.join(sync_map_audio_path_prefix,
                                             audio_name)
            audio_wav_path = os.path.join(
                tmp_dir,
                f'{drop_extension(audio_name)}_audio.wav')
            subprocess.run(['ffmpeg', '-n', '-i', audio_path, '-rf64', 'auto',
                            audio_wav_path])

            audio_mfcc_seq = np.ascontiguousarray(
        ...
        m = len(audio_mfcc_seq)

        _, path = c_FastDTWBD(text_mfcc_sequence, audio_mfcc_seq,
                              skip_penalty, radius=radius)
        ...
        text_path_anchor_indices = np.searchsorted(text_path_frames,
                                                   anchors_to_map)
        ...
        timings = ((np.append(anchors_matched_frames, audio_path_frames[-1])
                    + audio_start_frame) * 0.040)
        ...
        # Decide whether to process next file or to align the tail of the
        # current one
        ...
        if last_matched_audio_frame == m - 1 or not process_next_text:
            # Process next audio if there are no unmatched audio frames in the
            # tail or there are more text fragments to map, i.e. we choose to
            # process next audio if we cannot decide. This strategy is correct
            # if there are no extra fragments in the end.
            process_next_audio = True
        else:
            # Otherwise align tail of the current audio
            process_next_audio = False
            audio_mfcc_seq = audio_mfcc_seq[last_matched_audio_frame:]
...
def output_smil(sync_map, output_dir):
    ...
    for text_path, fragments in sync_map.items():
        ...
        file_path = os.path.join(
            output_dir,
            f'{drop_extension(text_name)}.smil'
        )
...
def output_json(sync_map, output_dir):
    for text_path, fragments in sync_map.items():
        ...
        file_path = os.path.join(
            output_dir,
            f'{drop_extension(text_name)}.json'
        )
        with open(file_path, 'w') as f:
...
def print_sync_map(sync_map):
    for text, fragment_map in sync_map.items():
        ...
        for fragment, info in fragment_map.items():
            print(fragment, info['audio_file'], info['begin_time'],
                  info['end_time'])
